...
import { User, Lock, Eye, EyeOff, FileText, Calendar, MessageSquare, Download, CheckCircle, Clock, AlertCircle } from 'lucide-react';
          ...
          <div className="bg-white p-8 rounded-xl shadow-lg">
            ...
            <div className="mt-6 text-center">
              <p className="text-sm text-gray-600">
                Não tem uma conta?{' '}
                <a href="/contato" className="text-primary-600 hover:text-primary-700 font-medium">
                  Entre em contato
                </a>
              </p>
            </div>
      ...
      <div className="container-max section-padding">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
                ...
                {projects.map((project) => (
                  <div key={project.id} className="border border-gray-200 rounded-lg p-6">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <span className={`px-3 py-1 rounded-full text-sm font-medium ${project.statusColor} bg-opacity-10`}>
                        {project.status}
                      </span>
                    </div>
            ...
            {/* Messages */}
            <div className="bg-white rounded-xl shadow-lg p-6">
              <h2 className="text-xl font-bold text-gray-900 mb-6">Mensagens Recentes</h2>
              <div className="space-y-4">
                {messages.map((message) => (
                  <div
                    key={message.id}
                    className={`p-4 rounded-lg border cursor-pointer hover:bg-gray-50 transition-colors ${
                      message.unread ? 'border-primary-200 bg-primary-50' : 'border-gray-200'
                    }`}
                  >
                    <div className="flex items-center justify-between mb-2">
                      <h3 className={`font-medium ${message.unread ? 'text-primary-900' : 'text-gray-900'}`}>
                        {message.from}
                      </h3>
                      <span className="text-sm text-gray-500">{message.date}</span>
                    </div>
                    <h4 className={`font-medium mb-1 ${message.unread ? 'text-primary-800' : 'text-gray-800'}`}>
                      {message.subject}
                    </h4>
